...
public class LogScheduler {
	...
	public void InsertDataByLog() {
		...
		for (GameInfo gi : findGames) {
			...
			List<InGameHuntLog> allHuntLogs = inGameHuntLogRepository.findInGameHuntLogsByGameId(gi.getId());
			for (InGameHuntLog hl : allHuntLogs) {
				Optional<MonsterStatistics> monsterStatistics = monsterStatisticsRepository.findById(
					hl.getCreatureId());
